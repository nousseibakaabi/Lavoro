...
const UpdateProject = () => {
  ...
  useEffect(() => {
    ...
    const startPicker = flatpickr(startDateRef.current, {
      ...
      defaultDate: projectData.start_date || "today",
    ...
    const endPicker = flatpickr(endDateRef.current, {
      enableTime: true,
      dateFormat: "Y-m-d H:i",
      defaultDate: projectData.end_date,
      onChange: (selectedDates) => {
        const dateStr = selectedDates[0]?.toISOString();
    ...
    // Set minimum end date if start date exists
    if (projectData.start_date && endPicker) {
      const minEndDate = new Date(projectData.start_date);
  ...
  }, [initialLoading, projectData.start_date, projectData.end_date]);
  ...
  const validateDates = (start, end) => {
    ...
    if (start && end) {
      ...
      if (endDate < minEndDate) {
        newErrors.end_date = 'End date must be at least 2 weeks after start date';
      } else {
        ...
        <div>
          <nav aria-label="breadcrumb">
            <ol className="breadcrumb mb-1">
              ...
              <li className="breadcrumb-item active" aria-current="page">Update Project</li>
            </ol>
          </nav>
