...
const TaskCommentsTab = ({ taskId, projectData }) => {
  ...
  useEffect(() => {
    const fetchCommentsAndCheckRole = async () => {
      try {
        ...
        // Fetch comments
        const commentsResponse = await axios.get(`http://localhost:3000/tasks/getComments/${taskId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleAddComment = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post('http://localhost:3000/tasks/comment', {
        task_id: taskId,
        content: newComment
      }, {
        headers: { Authorization: `Bearer ${token}` }
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://localhost:3000/tasks/deleteComment/${commentId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
                    ...
                    <img
                      src={
                        ...
                        comment.user_id.image.startsWith('https')
                          ? comment.user_id.image
                          : `http://localhost:3000${comment.user_id.image}`
                      }
