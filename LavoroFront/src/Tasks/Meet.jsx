...
const MeetComponent = () => {
  ...
  const fetchMeetings = async () => {
    try {
        const token = localStorage.getItem("token");
        const response = await axios.get('http://localhost:3000/meet/getAllMeet', {
            headers: {
                Authorization: `Bearer ${token}`,
            },
            withCredentials: true,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        await axios.post('http://localhost:3000/meet/addMeet', meetingData, {
            headers: {
                Authorization: `Bearer ${token}`,
            },
            withCredentials: true,
  ...
  const handleUpdateMeeting = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:3000/meet/updateMeet/${selectedEvent.id}`, meetingData, {
        headers: { Authorization: `Bearer ${token}` },
        withCredentials: true,
      });
  ...
  const handleDeleteMeeting = async () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const token = localStorage.getItem("token");
          await axios.delete(`http://localhost:3000/meet/deleteMeet/${selectedEvent.id}`, {
            headers: { Authorization: `Bearer ${token}` },
            withCredentials: true,
          });
          fetchMeetings();
