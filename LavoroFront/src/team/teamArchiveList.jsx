...
export default function TeamArchive() {
  ...
  const handleViewClick = (teamId) => {
    navigate(`/teams/teamArchiveDetails/${teamId}`);
  };
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const response = await fetch('http://localhost:3000/teams/export-archived', {
        method: 'GET',
        headers: {
  ...
  useEffect(() => {
    const fetchArchivedTeams = async () => {
      try {
        ...
        const response = await fetch('http://localhost:3000/teams/archived-teams', {
          headers: {
            'Authorization': `Bearer ${token}`
          },
        ...
        if (response.ok) {
          ...
          if (data.success && Array.isArray(data.data)) {
            setArchivedTeams(data.data);
          }
          // Check if the response is directly an array (old format)
  ...
  const handleUnarchive = async (teamId) => {
    try {
      const response = await fetch(`http://localhost:3000/teams/unarchive/${teamId}`, {
        method: 'POST',
        headers: {
      ...
      const data = await response.json();
      console.log('Unarchive response:', data); // Debug log
  
      if (response.ok) {
        if (data.archiveDeleted) {
          setArchivedTeams(prev => prev.filter(t => t._id !== teamId));
          Swal.fire('Success!', 'Team unarchived and removed from archive.', 'success');
        } else {
          Swal.fire('Warning!', 'Team was restored but archive cleanup failed.', 'warning');
        }
      } else {
        throw new Error(data.message || 'Failed to unarchive team');
      }
    } catch (error) {
  ...
  const handleDelete = async (teamId) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await fetch(`http://localhost:3000/teams/delete-archived-team/${teamId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
        ...
        if (response.ok) {
          ...
          Swal.fire("Error!", data.message || "Failed to delete the team.", "error");
        }
      } catch (error) {
