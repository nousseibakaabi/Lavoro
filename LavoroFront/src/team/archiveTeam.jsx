...
export default function TeamArchive() {
  ...
  useEffect(() => {
    const fetchArchivedTeams = async () => {
      try {
        const response = await fetch('http://localhost:3000/teams/archived', {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
  ...
  const handleUnarchive = async (teamId) => {
    try {
      const response = await fetch(`http://localhost:3000/teams/${teamId}/unarchive`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
  ...
  const handleDelete = async (teamId) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(`http://localhost:3000/teams/archived/${teamId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
